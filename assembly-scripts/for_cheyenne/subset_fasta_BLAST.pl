...
# It takes an input fastq and subsets the fastq based on the contigs that were present in the BLAST output
# It creates a second file telling the user how many unique query & unique hits

my ($fastq_file, $BLAST_file, $Out_fastq, $Out_stats) = @ARGV;

open(my $FASTQ, "<", $fastq_file) or die "Cannot open $fastq_file\n";
open(my $BLAST, "<", $BLAST_file) or die "Cannot open $BLAST_file\n";
open(my $OUT_FASTQ, ">", $Out_fastq) or die "Cannot create $Out_fastq\n";
